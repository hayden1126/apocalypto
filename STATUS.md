...
- **Load-bearing unknown, now partially answered.** Phone-scale PDR works for hundreds of metres (2-5%), but two questions remain before the multi-km architecture is committed: (1) does it survive PHONE-grade IMU noise (worse than this Xsens)? (2) can heading drift be BOUNDED over multi-km (periodic GNSS re-anchor, gated magnetometer, or landmark fix), since the naive magnetometer did not do it and unbounded heading kills the multi-km claim?
- **Next-step decision (pending Hayden):** SHL dataset (real phone IMU at scale, coarse phone-GPS truth, everyday-life segmentation) vs a self-collected multi-km phone walk (exact target sensor + motion + a logged GPS to test as the re-anchor). My lean: **self-collect one multi-km phone walk** because both open questions (phone noise at multi-km, bounded-heading via periodic GNSS re-anchor) are most directly answered by a controlled walk that plugs straight into the now-validated harness. Cost is the legwork (Hayden's).
