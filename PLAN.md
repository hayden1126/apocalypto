...
> **RESOLVED 2026-07-08 -> PASS.** The ~2 km out-and-back walk (`data/phone/ma_ling_2km`, merged to `main` via PR #2) landed. Phone PDR + trusted GNSS re-anchoring at ~15-20 s cadence holds street level: held-out radial P95 **6.7 m at 15 s**, 12.8 m at 20 s, 24.8 m at 30 s (marginal); cross-track P95 5.5 / 8.9 / 15.8 m. PASS gate met at <= 20 s cadence, robust across the honest k-band (leg k was contaminated by exaggerated strides -> k came from the band, ~0.41-0.54; verdict is insensitive because re-anchoring resets position each interval). Raw PDR drift is ~25% over 2 km (heading-dominated; out-and-back compounds it), so re-anchoring is essential. Full numbers + the calibration-leg lesson in `STATUS.md` "Findings (step 2 FULL ~2 km walk)". The next collection should add pose variation (B4) and a natural-pace calibration leg. Original blocker text kept below for the record.
...
- **Re-run on the ~2 km walk (2026-07-08): single-pass map-heading FAILS AGAIN, but the confound flipped and the diagnosis is now clean.** On `ma_ling_2km` (distinct-street out-and-back, k=0.537): frac_matched 0.10, **70 m median matched-edge snap**, map-heading (all) makes cross-track worse (177 -> 467 m); GNSS-15 s reference is fine (xtrack 3.5 m). Root cause is no longer the cramped venue but the **~25% raw drift**: single-pass matching of a heavily-drifted track snaps to the wrong edges everywhere (circular -- you need good heading to match, but you match to fix heading). The coverage split is the key evidence that the **mechanism is sound**: where the track snaps within 70 m (correct edge) map-heading cuts cross-track 67.6 -> 4.6 m; where it snaps far it explodes. So this is **still NOT a clean KILL of map-as-heading-bound** -- it kills *single-pass matching on raw drifted PDR*. The two viable directions are now sharp: (1) **iterate match<->correct** (re-match after each heading reset so the track converges toward the streets), or (2) **map-match a GNSS-re-anchored (~6 m) track** to refine heading between fixes. And note: on a GPS-available walk, re-anchoring alone already bounds drift, so the map's *heading* contribution is only measurable in GNSS-denied stretches -- which this walk lacks. The `map_heading_experiment.py` machinery is now parametrized (`--k`/`--name`/per-name OSM cache) and ready for both follow-ups.
...
- Unit + regression: `cd prototypes/pdr-benchmark && .venv/bin/python -m pytest -q` (59 tests: the 28 originals + Workstream A `trusted_fix`/checkpoint/phone-regression + B1 map-heading + 3 calibrate_leg + 8 Android frame-adapter from PR #9, all green).
