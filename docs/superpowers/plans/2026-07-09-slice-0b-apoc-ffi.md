...
> **Reconciliation note (2026-07-09, post-review):** this plan is the as-authored artifact; a 10-finding adversarial review after execution hardened the shipped code, so read the code for current truth. The shipped `dto` layer now owns DATA validation beyond shapes: strictly-increasing time bases for every `interp_ne` source (non-monotonic input used to return `APOC_OK` with garbage), finite-only coordinates and times (a NaN position silently passed the comparison-based gates and panicked the innovation path), and `lock_window >= 1` (a zero-initialized C params struct panicked core `lock_time`); all now return `APOC_ERR_INVALID_ARGS`. `TrustedFixParamsDto` was deleted: the `dto` wiring takes the core `TrustedFixParams` directly and the NaN==None convention lives only in `ApocTrustedFixParams` at the C edge. The buffer machinery is single-sourced through generic `into_raw_parts`/`free_raw_parts`; `pairs()` was replaced by zero-copy `as_chunks::<2>()` and the interp flatten by `Vec::into_flattened()`. The Safety contracts now require `out` to not overlap any input, and the `capi` module doc records the `panic = "unwind"` assumption behind the guard. Test counts also grew past the body's figures: execution split each concern into its own test, so `dto_wiring.rs` carries 4 tests and `capi_roundtrip.rs` 4 (not the 2/3 stated below); apoc-ffi ships 9 tests and the workspace 23, not 6/20. The code blocks below predate these fixes.
